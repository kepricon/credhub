...
public class V44_2__migrate_encypted_values_to_encryped_value_table implements
  ...
  public void migrate(JdbcTemplate jdbcTemplate) throws Exception {
    ...
    List<UUID> credentialsWithEncryptedValues = jdbcTemplate.queryForList("select uuid " +
        "from credential_version where encrypted_value is not null", UUID.class);

    for (UUID record : credentialsWithEncryptedValues) {
      Object uuid = makeUuid(databaseName);
      jdbcTemplate.update(
          "insert into encrypted_value values (select ?, encryption_key_uuid, encrypted_value, nonce, updated_at)" +
              "from credential_version where uuid is ?);",
          uuid, record);
      jdbcTemplate.update(
          "update credential_version set encrypted_value_id=? " +
          "where uuid = ?",
          uuid, record);
    }


    List<UUID> passwordsWithEncryptedValues = jdbcTemplate.queryForList("select uuid " +
        "from password_credential where encrypted_generation_parameters is not null", UUID.class);
