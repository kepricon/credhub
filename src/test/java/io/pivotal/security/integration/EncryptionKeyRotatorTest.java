...
public class EncryptionKeyRotatorTest {
  ...
  public void rotation_removesOldCanaries() throws Exception {
    ...
    setActiveKey(1);
    encryptionKeyRotator.rotate();
    List<UUID> knowAndInactiveKeys = encryptionKeyCanaryMapper.getCanaryUuidsWithKnownAndInactiveKeys();
    List<EncryptionKeyCanary> allCanaries = encryptionKeyCanaryDataService.findAll();
    for( EncryptionKeyCanary canary : allCanaries ) {
      for( UUID uuid : knowAndInactiveKeys ) {
        assertThat(canary.getUuid(), not(equalTo(uuid)));
      }
    }
    List<UUID> uuids = allCanaries.stream().map(EncryptionKeyCanary::getUuid).collect(Collectors.toList());
    assertThat(uuids, hasItem(encryptionKeyCanaryMapper.getActiveUuid()));
  }
